...
__n_steps = 6400 # timesteps - longest sentence
...
def get_book(name, language):
    ...
    book = [row[0] for row in csv.reader(lzma.open(name+'.xz', 'rt'))]
...
pat = get_book('claims.csv', english)
...
pat_pred = RNN(transform_block(pat_chars), pat_lens, 'pat', None)
